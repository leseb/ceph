			 ...
			 Context *onfreespace)
{
  ...
  if (onfreespace) {
    if (block_writes_upfront || max_dirty == 0) {
      onfreespace->complete(0);
    } else {
      finisher.queue(new C_WaitForWrite(this, bytes_written, onfreespace));
    }
  } else {
    assert(!block_writes_upfront);
  }
...
void ObjectCacher::C_WaitForWrite::finish(int r)
{
  ...
  m_oc->maybe_wait_for_write(m_len);
  m_onfinish->complete(r);
}

void ObjectCacher::maybe_wait_for_write(uint64_t len)
{
  assert(lock.is_locked());
